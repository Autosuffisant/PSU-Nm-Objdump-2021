...
uint32_t get_flag_32(elf_data_t *elf)
{
    ...
    for (unsigned int i = 0; i < elf->ehdr_32->e_shnum; i++)
        if (elf->shdr_32[i].sh_type != SHT_SYMTAB
        || elf->shdr_32[i].sh_type != SHT_DYNAMIC)
            is_symbol_flag = true;
    if (is_symbol_flag == true)
        flag |= HAS_SYMS;
    if (elf->ehdr_32->e_type == ET_EXEC)
        flag |= EXEC_P;
    if (elf->ehdr_32->e_type == ET_REL)
        flag |= HAS_RELOC;
    if (elf->ehdr_32->e_phnum != 0)
        flag |= D_PAGED;
    if (elf->ehdr_32->e_type == ET_DYN)
        flag |= DYNAMIC;
    return flag;
}
